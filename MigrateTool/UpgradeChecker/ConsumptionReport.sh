#!/bin/bash
# Artifact Extractor
#This script is for Customer: ConsumptionReport
...
then
        clear
        echo "####################################################"
        echo "Usage: ./ConsumptionReport.sh -downloadDir <dwnldDir>"
        echo "example: ./ConsumptionReport.sh -downloadDir TmpDir"
        echo "####################################################"
        exit 65
fi
...
downloadArtifacts(){
	...
	#THIS IS FOR ConsumptionReport only
	grep lnk ConsumptionReport/import-job/*.json | grep -Po '\d*.json' | grep -Po '\d*' > DL.id
	while IFS='' read ID
        do
        	echo "Executing ${ID}"
